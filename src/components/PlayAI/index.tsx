...
import { FaArrowAltCircleLeft, FaCircleNotch, FaDiceD6 } from "react-icons/fa";
...
export function PlayAI() {
  ...
  const [bot] = useState(() => new botAI(Player2.id));
  ...
  const turnBot = () => {
    const botChoose = bot.chooseLevel(dice, game, Player2.name);
    if (botChoose != null) {
      addDice(botChoose.line, botChoose.dice, botChoose.player);
    }
  };

  useEffect(() => {
    if (playerDice && rollEnd && !end) {
  ...
  const addDice = (line: number, dice: number, player: string) => {
